...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!message.member.hasPermission("BAN_MEMBERS")) return message.quote("** Você precisa ter permissão de `Banir Membros` para executar este comando**").then(m => m.delete({ timeout: 60000 }))
        if (!message.guild.me.hasPermission("BAN_MEMBERS")) return message.quote("** Eu preciso da permissão de `Banir Membros` para executar este comando**").then(m => m.delete({ timeout: 60000 }))
