...
module.exports = {
    name: "addrole",
    aliases: ["adiocionarrole"],
    decription: "Comando usado para adicionar uma role ao user mencionado",
    usage: "addrole <@usuario> <@cargo>",
    run: async (client, message, args) => {
        if (!message.channel.permissionsFor(client.user.id).has('SEND_MESSAGES')) return error.permissionFor(message)
        if (!message.member.hasPermission("MANAGE_ROLES")) return message.quote("** Você precisa ter permissão de `Gerenciar Cargos` para executar este comando**").then(m => m.delete({ timeout: 60000 }))
        if (!message.guild.me.hasPermission("MANAGE_ROLES")) return message.quote("** Eu preciso da permissão de `Gerenciar Cargos` para executar este comando**").then(m => m.delete({ timeout: 60000 }))
        ...
        if (member.roles.cache.has(role.id)) return error.userRole(message)

        member.roles.add(role)
        const embed = new MessageEmbed()
            .setAuthor(`Sistema De Punições - ${client.user.username} - AddRole`, client.user.displayAvatarURL())
            .setColor("#FF0000")
            .addField("Punido", `\`${member.user.tag}\``)
            .addField("Punido ID", `\`${member.id}\``)
            .addField("Punido Por", `\`${author.tag}\``)
            .addField("Autor ID", `\`${author.id}\``)
            .addField("Motivo", `\`${motivo || "Não Especificado"}\``)
